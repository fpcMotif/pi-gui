...
export interface OmpSidecarHandle {
	...
	stop: () => Promise<void>;
	/**
	 * Web-stream views of the child's stdio. Provided for parity with the ACP
	 * client, but the RPC connection uses the raw Node `Readable`/`Writable`
	 * directly — line-oriented NDJSON doesn't need WHATWG stream adapters,
	 * which is what avoids the known Bun-on-Windows handshake hang that ACP's
	 * `ndJsonStream` hit.
	 */
